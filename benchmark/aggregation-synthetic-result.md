...
evidence that the normalized aggregation gap is stable under severe dropout.
...
- Ignored full bundle: 501 files and 68,034,582 bytes. `artifacts.tsv` SHA-256
  ...
  A binary replay verified all 486 checkpoint fingerprints. It recomputed the
  endpoints, folds, predictions, coefficients, and bootstrap values. The
  maximum text-transport delta was below `5e-16`. All 14 listed artifact hashes
  matched.
...
- The marginal median observed-gap rise from dropout 0 to 0.5 was 0.09762. The
  corresponding rise from planted complementarity 0 to 1 was 0.10728. Thus,
  technical dropout is comparable with the planted effect, as PLAN anticipated.
...
aggregate-then-score behavior, but not a general dropout-stability or biological
